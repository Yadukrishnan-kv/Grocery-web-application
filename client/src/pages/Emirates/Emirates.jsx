...
const Emirates = () => {
  ...
  const handleEditSave = async () => {
    const errors = {};
    if (!editFormData.emiratesName.trim()) errors.emiratesName = 'Emirates name is required';
    if (!editFormData.emiratesCode.trim()) errors.emiratesCode = 'Emirates code is required';
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to update Emirates');
    } finally {
      setEditSaving(false);
    }
  };
        ...
        <div className="emirates-page-wrapper">
          ...
          <div className="emirates-list-card">
            <div className="emirates-list-header">
              <div>
                <h2>Emirates List</h2>
                <p className="emirates-list-sub">Manage all emirates regions</p>
              </div>
              <button
                className="emirates-add-btn"
                onClick={() => navigate('/emirates/create')}
              >
                + Add Emirates
              </button>
            </div>
        ...
        <div className="emirates-modal-overlay" onClick={() => setShowEditModal(false)}>
          <div className="emirates-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="emirates-modal-actions">
              <button className="emirates-cancel-btn" onClick={() => setShowEditModal(false)} disabled={editSaving}>
                Cancel
              </button>
              <button className="emirates-submit-btn" onClick={handleEditSave} disabled={editSaving}>
